...
public class StepDetector implements SensorEventListener {
    ...
    private List<OnStepDetected> listeners = new ArrayList<>();
    ...
    public void addListener(OnStepDetected listener)
    {
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        synchronized (this) {
            if (sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
                ...
                for (int i=0 ; i<3 ; i++) {
                    float v = mYOffset + event.values[i] * mScale[1];
                    vSum += v;
                }
...
//                            Log.i(TAG, "step");
                            ...
                            for (OnStepDetected listener: listeners) {
                                listener.onStep(0);
                            }
                            mLastMatch = extType;
                        }
